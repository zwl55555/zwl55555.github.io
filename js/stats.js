...
;(function (h, n) {
  ...
  function q(c, a, b) {
    var d = window.location.host,
      ...
      f = d.split(".")
    2 < f.length &&
      (d = (e[f.slice(-2).join(".")] ? f.slice(-3) : f.slice(-2)).join("."))
    document.cookie =
      c + "=" + a + ";path=/;domain=" + d + (b ? ";expires=" + b : "")
  }
  function l(c) {
    var a,
      b,
    ...
    void 0 === c
      ...
      : ((d =
        ...
        (b = d[3]),
        (d = d[4]))
    void 0 !== d && (d = encodeURI(d.replace(/\"|\'|\<|\>/gi, "M")))
    if (b)
  ...
  function y() {
    try {
      var c = navigator,
        a = screen || { width: "", height: "", colorDepth: "" },
        b = document.body,
      ...
      b &&
        b.addBehavior &&
        (b.addBehavior("#default#clientCaps"), (a = b.connectionType))
      var b = { fl: "", scr: d, scl: e, lg: f, jv: g, tz: h, ct: a },
        m,
        k,
        l,
        n
      if ((m = c.plugins) && (k = m.length))
        ...
        (n = new ActiveXObject("ShockwaveFlash.ShockwaveFlash").GetVariable(
          "$version"
        )),
  ...
  function A() {
    ...
    for (a in k) {
      var b = p(k[a].c_id),
        d
      "afs" == a
        ? (d = (d = /ssid=([^&]*)/i.exec(l().hash)) && d[1] ? d[1] : "")
        : ((d = l().param), (d = d[k[a].id] ? d[k[a].id] : ""))
      d
        ? (c.push("ty=" + k[a].key + ";ck=0;id=" + d),
          (b = new Date()),
          b.setTime(b.getTime() + 2592e6),
          q(k[a].c_id, d, b.toGMTString()))
        : b && c.push("ty=" + k[a].key + ";ck=1;id=" + b)
    }
    return { pf: c.join("|") }
  }
  function u(c) {
  ...
  function B(c) {
      ...
      e = function () {
        t(
          ("https:" == document.location.protocol ? "https://" : "http://") +
            "tajs.qq.com/ping_hotclick_min.js",
          function () {
    ...
    1 * h.pf
      ? new RegExp(b).test(h.hot.url) && e()
      : ((a = h.sid),
        t(
          "http://tcss.qq.com/heatmap/" +
            (a % 100) +
            "/" +
            v(a) +
            ".js?random=" +
  ...
  function C(c) {
    ...
    document.write(
      [
        '<a href="http://ta.qq.com?ADTAG=FROUM.FOOTER.CLICK.ICON" title="\u817e\u8baf\u5206\u6790" target="_blank">',
        a[c] || a.df,
        "</a>",
      ].join("")
    )
